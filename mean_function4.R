run <- function(
    nrep = 2000,
    N_vec = c(200, 500, 800),
    model = c("latent", "composite", "scaled", "latent_basis"),
...
'
  ...
  # ================================================================
  # 4. Population truth for composite models
  # ================================================================
  s_t_pop <- colSums(lam_mat) / J   # mean of loadings per time
  c_t_pop <- colSums(tau_mat) / J   # mean of intercepts per time
  E_St    <- c_t_pop + s_t_pop * (mu_alpha + t_idx * mu_beta)
  
  ## Quadratic true values:
  B_quad <- cbind(1, t_idx, t_idx^2)
  quad_true_coefs <- as.numeric(solve(crossprod(B_quad), crossprod(B_quad, E_St)))

  ## Latent basis true values:
  b_true <- (E_St - E_St[1]) / (E_St[TT] - E_St[1])
  
  ## Latent basis model k1, k2 truth:
  lb_k1_true <- E_St[1]
  ...
  for (n_idx in seq_along(N_vec)) {
    ...
    for (i in seq_len(nrep)) {
      ...
      # (B) Data preparation
      if (model %in% c("composite", "quadratic", "latent_basis")) {
        dat_fit <- data.frame(
      ...
      recall[i, "phi11.cov"]  <- ci_in(recall[i, "phi11"],  recall[i, "phi11.SE"], var_alpha)
      recall[i, "phi12.cov"]  <- ci_in(recall[i, "phi12"],  recall[i, "phi12.SE"], cov_ab)
      recall[i, "cor12.cov"]  <- ci_in(recall[i, "cor12"],  recall[i, "cor12.SE"], cor_ab)
      recall[i, "phi22.cov"]  <- ci_in(recall[i, "phi22"],  recall[i, "phi22.SE"], var_beta)
      recall[i, "kappa1.cov"] <- ci_in(recall[i, "kappa1"], recall[i, "kappa1.SE"], mu_alpha)
      recall[i, "kappa2.cov"] <- ci_in(recall[i, "kappa2"], recall[i, "kappa2.SE"], mu_beta)
    ...
    summary_row <- data.frame(
      ...
      ## Bias (against generating values)
      ...
      rho_IS.m = mean(
        recall_ok$phi12 / (sqrt(recall_ok$phi11) * sqrt(recall_ok$phi22)),
        na.rm = TRUE),
      d_slope.m = mean(
        recall_ok$kappa2 / sqrt(recall_ok$phi22),
        na.rm = TRUE),
