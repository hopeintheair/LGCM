...
'
    ...
    ## residual variance block (invariant)
    resvar_inv <- '
...
'
  ...
  ## label residual variances for 1-LGCM models (latent already labeled s1..s5)
  if (model != "latent") {
    lav_model <- paste0(lav_model, '
time1 ~~ s1*time1
time2 ~~ s2*time2
time3 ~~ s3*time3
time4 ~~ s4*time4
...
')
  ...
  ## phi truth by model
  phi_scale <- switch(model, "sum" =, "quadratic" =, "latent_basis" = mean(a_t_sum)^2,
  ...
  for (n_idx in seq_along(N_vec)) {
    ...
    for (i in seq_len(nrep)) {
      ...
      ## sigma2_bar: average residual variance via labels s1..sT
    ...
    summary_row <- data.frame(
      ...
      slope_rel.m = mean( recall_ok$phi22 /
                          (recall_ok$phi22 + recall_ok$sigma2_bar / SS_t), na.rm = TRUE),
