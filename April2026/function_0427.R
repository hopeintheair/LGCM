...
'
  ...
  if (model == "latent") {
    ...
    ## Sstructural part (2nd-order LGCM + disturbances)
    lgcm_struct <- '
# Second-order LGCM
...
'
    ...
    ## residual variance block 
   if (mni_location == "residual") {
     resvar_inv <- '
# Residual variances (free)
i1t1 ~~ e11*i1t1; i1t2 ~~ e12*i1t2; i1t3 ~~ e13*i1t3; i1t4 ~~ e14*i1t4; i1t5 ~~ e15*i1t5
i2t1 ~~ e21*i2t1; i2t2 ~~ e22*i2t2; i2t3 ~~ e23*i2t3; i2t4 ~~ e24*i2t4; i2t5 ~~ e25*i2t5
...
'
    ...
    } else if (mni_location == "intercept") {
      meas_part <- '
# Measurement model (fixed)
f1 =~ 1*i1t1 + l2*i2t1 + l3*i3t1 + l4*i4t1 + l5*i5t1
f2 =~ 1*i1t2 + l2*i2t2 + l3*i3t2 + l4*i4t2 + l5*i5t2
...
'
    ...
    ### 2.2. Sum-score LGCM models ====
  } else {
    ## Default: linear LGCM (sum score)
    lav_model <- '
