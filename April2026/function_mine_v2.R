...
.lav_admissible <- function(fit) {
  ...
  if (is.null(pe) || nrow(pe) == 0L) return(FALSE)
  if (any(is.na(pe$est))) return(FALSE)
  is_var <- pe$op == "~~" & pe$lhs == pe$rhs
  if (any(is_var & pe$est < 0, na.rm = TRUE)) return(FALSE)
  is_cov_like <- pe$op == "~~"
...
run <- function(
  ...
  ## drift properties: linear change
  t_idx  <- 0:(TT - 1)
  drift_t <- t_idx / (TT - 1)
  ...
  # rtruncnorm returns NaN with sd = 0 → fall back to constant when sd.lambda = 0
  ...
  ## Measurement Non-Invariance (data-generating side)
...
'
  cu_sums <- '
# Correlated residuals (composite)
...
'
    ...
    ## ----- Measurement model blocks (loading + intercept) -----
    # Fully invariant: used when no MNI, or when MNI is on residuals
    meas_inv_full <- '
# Measurement model (loadings invariant)
...
'
    ...
    # Loading MNI - weak: only item 5 has time-varying loadings
    meas_load_weak <- '
# Measurement model (item 5 loadings free across time)
...
'
    ...
    # Loading MNI - strong: items 3, 4, 5 have time-varying loadings
    meas_load_strong <- '
# Measurement model (items 3,4,5 loadings free across time)
...
'
    ...
    # Intercept MNI - weak: only item 5 has time-varying intercepts
    meas_int_weak <- '
# Measurement model (loadings invariant)
f1 =~ 1*i1t1 + l2*i2t1 + l3*i3t1 + l4*i4t1 + l5*i5t1
f2 =~ 1*i1t2 + l2*i2t2 + l3*i3t2 + l4*i4t2 + l5*i5t2
...
# Intercepts (item 5 free across time)
i1t1 ~ 1*1; i1t2 ~ 1*1; i1t3 ~ 1*1; i1t4 ~ 1*1; i1t5 ~ 1*1
i2t1 ~ int2*1; i2t2 ~ int2*1; i2t3 ~ int2*1; i2t4 ~ int2*1; i2t5 ~ int2*1
i3t1 ~ int3*1; i3t2 ~ int3*1; i3t3 ~ int3*1; i3t4 ~ int3*1; i3t5 ~ int3*1
i4t1 ~ int4*1; i4t2 ~ int4*1; i4t3 ~ int4*1; i4t4 ~ int4*1; i4t5 ~ int4*1
...
'
    ...
    # Intercept MNI - strong: items 3, 4, 5 have time-varying intercepts
    meas_int_strong <- '
# Measurement model (loadings invariant)
f1 =~ 1*i1t1 + l2*i2t1 + l3*i3t1 + l4*i4t1 + l5*i5t1
f2 =~ 1*i1t2 + l2*i2t2 + l3*i3t2 + l4*i4t2 + l5*i5t2
...
# Intercepts (items 3,4,5 free across time)
i1t1 ~ 1*1; i1t2 ~ 1*1; i1t3 ~ 1*1; i1t4 ~ 1*1; i1t5 ~ 1*1
i2t1 ~ int2*1; i2t2 ~ int2*1; i2t3 ~ int2*1; i2t4 ~ int2*1; i2t5 ~ int2*1
...
'
    ...
    ## ----- Residual variance blocks -----
    # Fully invariant residuals
    resvar_inv_full <- '
...
'
    ...
    # Residual MNI - weak: only item 5 has time-varying residuals
    resvar_res_weak <- '
# Residual variances (item 5 free across time)
i1t1 ~~ e1*i1t1; i1t2 ~~ e1*i1t2; i1t3 ~~ e1*i1t3; i1t4 ~~ e1*i1t4; i1t5 ~~ e1*i1t5
i2t1 ~~ e2*i2t1; i2t2 ~~ e2*i2t2; i2t3 ~~ e2*i2t3; i2t4 ~~ e2*i2t4; i2t5 ~~ e2*i2t5
i3t1 ~~ e3*i3t1; i3t2 ~~ e3*i3t2; i3t3 ~~ e3*i3t3; i3t4 ~~ e3*i3t4; i3t5 ~~ e3*i3t5
i4t1 ~~ e4*i4t1; i4t2 ~~ e4*i4t2; i4t3 ~~ e4*i4t3; i4t4 ~~ e4*i4t4; i4t5 ~~ e4*i4t5
...
'
    ...
    # Residual MNI - strong: items 3, 4, 5 have time-varying residuals
    resvar_res_strong <- '
# Residual variances (items 3,4,5 free across time)
i1t1 ~~ e1*i1t1; i1t2 ~~ e1*i1t2; i1t3 ~~ e1*i1t3; i1t4 ~~ e1*i1t4; i1t5 ~~ e1*i1t5
i2t1 ~~ e2*i2t1; i2t2 ~~ e2*i2t2; i2t3 ~~ e2*i2t3; i2t4 ~~ e2*i2t4; i2t5 ~~ e2*i2t5
...
'
    ...
    # Always label time residuals s1..s5 so sigma2_bar matching works regardless of rho
    lav_model <- '
...
time1 ~~ s1*time1; time2 ~~ s2*time2; time3 ~~ s3*time3; time4 ~~ s4*time4; time5 ~~ s5*time5
'
  ...
  # Sum
  a_t_sum <- colSums(lam_mat)
  c_t_sum <- colSums(tau_mat)
  ...
  for (n_idx in seq_along(N_vec)) {
    ...
    for (i in seq_len(nrep)) {
      ...
      # latent  -> mean of factor disturbances (s1..s5)
      # composite -> mean of time residuals (s1..s5)
    ...
    summary_row <- data.frame(
      ...
      rho_IS      = mean(recall_ok$phi12 / (sqrt(recall_ok$phi11) * sqrt(recall_ok$phi22)), na.rm = TRUE),
      d_slope     = mean(recall_ok$kappa2 / sqrt(recall_ok$phi22), na.rm = TRUE),
      std_tot_eff = mean(recall_ok$kappa2 * (TT - 1) / sqrt(recall_ok$phi11), na.rm = TRUE),
      slope_rel   = mean(recall_ok$phi22 / (recall_ok$phi22 + recall_ok$sigma2_bar / SS_t), na.rm = TRUE),
