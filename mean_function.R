run <- function(
  ...
  theta_base <- var_eta1 * (1 / target_loading^2 - 1)
  ...
  if (length(noninv_items) > 0L) {
    if (mni_location == "intercept") {
      ## tau_jt = tau_base + d_tau * t_prop
      for (t in seq_len(TT)) {
        tau_mat[noninv_items, t] <- tau_vec[noninv_items] + mni_params$d_tau * t_prop[t]
      }
      
    } else if (mni_location == "loading") {
      ## lambda_jt = lambda_base + d_lam * t_prop
      for (t in seq_len(TT)) {
