run5i3t <- function(nrep, model = c("latent", "composite"), 
                    ...
                    auto_var1 = TRUE, seed = 123, rho = 0, I = 5, tlen = 3,
  ...
  if (model %in% c("latent", "composite")) {
    if (I != 5L || tlen != 3L) {
      stop("This mode is for I = 5, tlen = 3. ")
    }
  }
  ...
  if (model == "latent") {
    ...
    for (i in seq_len(nrep)) {
      ...
      } # else: psi1~psi3 그대로 사용
      
      psi.mtx <- matrix(c(psi1^2, 0, 0, 
                          0, psi2^2, 0,
                          0, 0, psi3^2), nrow = 3L, byrow = TRUE)
      psi_disturb <- MASS::mvrnorm(nsize, mu = c(0,0,0), Sigma = psi.mtx)
      ...
      eta_mat <- cbind(eta1, eta2, eta3)  # nsize x tlen
  ...
  i1t1 ~~ (sig1)*i1t1; i1t2 ~~ (sig1)*i1t2; i1t3 ~~ (sig1)*i1t3; 
  i2t1 ~~ (sig2)*i2t1; i2t2 ~~ (sig2)*i2t2; i2t3 ~~ (sig2)*i2t3; 
  i3t1 ~~ (sig3)*i3t1; i3t2 ~~ (sig3)*i3t2; i3t3 ~~ (sig3)*i3t3; 

  i4t1 ~~ (sig41)*i4t1; i4t2 ~~ (sig42)*i4t2; i4t3 ~~ (sig43)*i4t3; 
  i5t1 ~~ (sig51)*i5t1; i5t2 ~~ (sig52)*i5t2; i5t3 ~~ (sig53)*i5t3; 
  ...
  if (model == "composite") {
    ...
    for (i in seq_len(nrep)) {
      ...
      data.comp <- data.frame(time1,time2,time3)
    ...
    recall.c <- dplyr::mutate(
      ...
      phi12.power  = mean(phi12.mark, na.rm = TRUE),
      phi22.power  = mean(phi22.mark, na.rm = TRUE),
      alpha1.power = mean(alpha1.mark, na.rm = TRUE),
      alpha2.power = mean(alpha2.mark, na.rm = TRUE),
      
      phi11.cov.rate = mean(phi11.cov, na.rm = TRUE),
      phi12.cov.rate = mean(phi12.cov, na.rm = TRUE),
      phi22.cov.rate = mean(phi22.cov, na.rm = TRUE),
      alpha1.cov.rate= mean(alpha1.cov, na.rm = TRUE),
      alpha2.cov.rate= mean(alpha2.cov, na.rm = TRUE),
      ...
      phi12.bias   = mean(phi12 - phi12p),
      phi22.bias   = mean(phi22 - phi2p),
      alpha1.bias  = mean(alpha1 - k1p),
      alpha2.bias  = mean(alpha2 - k2p),
      
      phi11.R.bias = mean((phi11 - phi1p)/phi1p),
      phi12.R.bias = mean((phi12 - phi12p)/phi12p),
      phi22.R.bias = mean((phi22 - phi2p)/phi2p),
      alpha1.R.bias= mean((alpha1 - k1p)/k1p),
      alpha2.R.bias= mean((alpha2 - k2p)/k2p),
      
      phi11.rmse   = sqrt(mean((phi11 - phi1p)^2)),
      phi12.rmse   = sqrt(mean((phi12 - phi12p)^2)),
      phi22.rmse   = sqrt(mean((phi22 - phi2p)^2)),
      alpha1.rmse  = sqrt(mean((alpha1 - k1p)^2)),
      alpha2.rmse  = sqrt(mean((alpha2 - k2p)^2)),
      
      phi11.rel.rmse = sqrt(mean((phi11 - phi1p)^2)) / phi1p,
      phi12.rel.rmse = sqrt(mean((phi12 - phi12p)^2)) / phi12p,
      phi22.rel.rmse = sqrt(mean((phi22 - phi2p)^2)) / phi2p,
      alpha1.rel.rmse = sqrt(mean((alpha1 - k1p)^2)) / k1p,
      alpha2.rel.rmse = sqrt(mean((alpha2 - k2p)^2)) / k2p,
      
      phi11.coverage = mean((phi11 - 1.96 * phi11.SE <= phi1p) & (phi11 + 1.96 * phi11.SE >= phi1p), na.rm = TRUE),
      phi12.coverage = mean((phi12 - 1.96 * phi12.SE <= phi12p) & (phi12 + 1.96 * phi12.SE >= phi12p), na.rm = TRUE),
      phi22.coverage = mean((phi22 - 1.96 * phi22.SE <= phi2p) & (phi22 + 1.96 * phi22.SE >= phi2p), na.rm = TRUE),
      alpha1.coverage = mean((alpha1 - 1.96 * alpha1.SE <= k1p) & (alpha1 + 1.96 * alpha1.SE >= k1p), na.rm = TRUE),
      alpha2.coverage = mean((alpha2 - 1.96 * alpha2.SE <= k2p) & (alpha2 + 1.96 * alpha2.SE >= k2p), na.rm = TRUE),
