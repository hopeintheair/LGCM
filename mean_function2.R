run <- function(
  ...
  ## Common parts: phi.mtx, nu_mat_pop, EC-population
  Sigma_ab <- matrix(c(var_alpha, cov_ab,
  ...
  ## non-invariant item location (the last n items)
  ...
  ## Non-invariance: linear change
  t_prop <- (seq_len(TT) - 1L) / (TT - 1L)   # 0, 0.25, 0.50, 0.75, 1.0     
  
  ## Time basis
  basis <- (seq_len(TT) - 1L) / (TT - 1L)
  
  ## Parameter matrices
  nu_base      <- rep(tau, I)       # base intercept
  nu_mat <- matrix(nu_base, nrow = I, ncol = tlen)
  
  tau_mat <- matrix(rep(tau_vec, TT), nrow = J)
  lambda_mat <- matrix(rep(lambda_vec, TT), nrow = J)
  ...
  if (length(noninv_items) > 0L) {
    if (mni_location == "intercept") {
      ...
      nu_mat <- matrix(rep(tau, tlen), nrow = I, ncol = tlen)
      
      for (tt in seq_len(TT)) {
        nu_mat[noninv_items, tt] <- rep(tau, times=I)[noninv_items] + nu_drift_max * drift_t[tt]
      }
      
    } else if (mni_location == "loading") {
      for (t in seq_len(TT)) {
        lambda_mat[noninv_items, t] <- lambda_vec[noninv_items] + mni_params$d_lam * t_prop[t]
      }
      
    } else if (mni_location == "residual") {
      ## theta_jt = theta_base * (1 + d_res * t_prop)
      for (t in seq_len(TT)) {
        theta_mat[noninv_items, t] <- theta_vec[noninv_items] * (1 + mni_params$d_res * t_prop[t])
      }
    }
  }
  ...
  # 0. Fixed settings 
  lambda_vec  <- rep(lambda_un, J)
  tau_vec     <- rep(tau_un, J)
  time_scores <- 0:(TT - 1)
  
  ## Residual variance:
  var_eta1   <- var_alpha + psi
  theta_base <- lambda_vec^2 * var_eta1 * (1 / target_loading^2 - 1)
  theta_vec <- rep(theta_base, J)
...
'
  ...
  ## Residual covariance with unit variance
  Sigma_unit <- make_resid_cov(1, TT, rho)
  ...
  for (n_idx in seq_along(N_vec)) {
    ...
    for (i in seq_len(nrep)) {
      ...
      # (A) Data Generation 
      ## Growth factors
      ab <- MASS::mvrnorm(N, mu = c(mu_alpha, mu_beta), Sigma = Sigma_ab)
      alpha_i <- ab[, 1L]
      beta_i <- ab[, 2L]
      ...
      for (t in seq_len(TT)) {
        zeta_t <- rnorm(N, 0, sqrt(psi))
        eta_mat[, t] <- alpha_i + beta_i * time_scores[t] + zeta_t
      }
      ...
      for (j in seq_len(J)) {
        ...
        for (t in seq_len(TT)) {
          dat_mat[, (t - 1L) * J + j] <-
            tau_mat[j, t] + lambda_mat[j, t] * eta_mat[, t] + E_j[, t]
        }
      }
