...
run <- function(
    ...
    ## Measurement Non-Invariance (MNI) ----
    ## location:  tau / lambda
  ...
  if (length(noninv_items) > 0L) {
    if (mni_location == "intercept") {
      ...
      for (t in seq_len(TT)) {                             # multiplicative
...
'
    ...
    ## Correlated uniqueness part
    cu_items <- '
# Correlated uniqueness (AR-Lag 1)
...
'
    ...
    ### 2.2. Effect-coding fitted 2-LGCM #####
  } else if (model == "EC_latnent") { 
    
    ## Structural part (effect-coded)
    lgcm_struct_EC <- '
  ...
  '
    ...
    ## Measurement part (strict MI + EC loadings & intercepts)
    meas_inv_EC <- '
  # Measurement model
  f1 =~ lmb1*i1t1 + lmb2*i2t1 + lmb3*i3t1 + lmb4*i4t1 + lmb5*i5t1
  f2 =~ lmb1*i1t2 + lmb2*i2t2 + lmb3*i3t2 + lmb4*i4t2 + lmb5*i5t2
  f3 =~ lmb1*i1t3 + lmb2*i2t3 + lmb3*i3t3 + lmb4*i4t3 + lmb5*i5t3
  f4 =~ lmb1*i1t4 + lmb2*i2t4 + lmb3*i3t4 + lmb4*i4t4 + lmb5*i5t4
  f5 =~ lmb1*i1t5 + lmb2*i2t5 + lmb3*i3t5 + lmb4*i4t5 + lmb5*i5t5
  ...
  '
    ...
    ## EC_latent only fitted under strict MI
    if (mni_size == "none" || mni_location == "none") {
      meas_part  <- meas_inv_EC
      resvar_inv <- resvar_inv_EC
    } else { return(NULL) }

    lav_model <- paste0(meas_part, resvar_inv, lgcm_struct_EC, ec_constraints)
    if (rho != 0) lav_model <- paste0(lav_model, cu_items)
    
  } else {
    
    ### 2.3. Sum-score 1-LGCM #####
    lav_model <- '
...
'
  ...
  raw_mean_cols <- paste0("m.", col_names)
  p_names <- c(p_base, raw_mean_cols)
  ...
  for (n_idx in seq_along(Nsize)) {
    ...
    for (i in seq_len(nrep)) {
      ...
      ## Track raw indicator means at every replication 
      recall[i, raw_mean_cols] <- colMeans(dat)
