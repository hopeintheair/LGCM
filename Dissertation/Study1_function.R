...
# Calculation of EC-fitted population
source('Study1_calculation.R')
...
run <- function(
      ...
      ## Design ----
      TT = c(5,8),    # time points
      J = c(5,8),     # number of indicators / time
    ...
    k1_true <- ec_pop$means[1L];   k2_true <- ec_pop$means[2L]
    phi1_true <- ec_pop$covs[1L,1L]; phi12_true <- ec_pop$covs[2L,1L]; 
    phi2_true <- ec_pop$covs[2L,2L]
    ...
    for (n_idx in seq_along(Nsize)) {
      ...
      summary_row <- data.frame(
        ...
        ## Relative RMSE
        phi11.rel.rmse  = sqrt(mean((recall_ok$phi11 - phi1_true)^2)) / phi1_true,
        phi12.rel.rmse  = sqrt(mean((recall_ok$phi12 - phi12_true)^2)) / phi12_true,
        phi22.rel.rmse  = sqrt(mean((recall_ok$phi22 - phi2_true)^2)) / phi2_true,
        kappa1.rel.rmse = sqrt(mean((recall_ok$kappa1 - k1_true)^2)) / k1_true,
        kappa2.rel.rmse = sqrt(mean((recall_ok$kappa2 - k2_true)^2)) / k2_true,
