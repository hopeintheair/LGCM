...
if (is.na(job_id)) stop("Usage: Rscript run_newer.R <job_id>")

# ── Project path (set in submit.sh via PROJECT_DIR env var) ─────────────────
project_dir <- Sys.getenv("PROJECT_DIR", unset = ".")
setwd(project_dir)

source("function_newer.R")
library(lavaan)
library(MASS)
library(dplyr)

# ── Load grid row ────────────────────────────────────────────────────────────
cond <- read.csv("conditions_newer.csv", stringsAsFactors = FALSE)
...
# ── Save output ──────────────────────────────────────────────────────────────
dir.create("results_newer", showWarnings = FALSE)

out_file <- sprintf(
  "results_newer/job%03d_%s_N%d_%s_rho_%s.RData",
  row$job_id,
  row$model,
  row$N_size,
  row$loading_quality,
